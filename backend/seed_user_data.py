...
def seed_user_data(user_id=1):
    db = get_db()
    
    print(f"Starting seeding process for User ID: {user_id}...")
    
    # ── 0. Delete existing records for this user to avoid conflicts and get a clean seed ──
    print("Deleting old records for this user...")
    try:
        db.table("purchase_orders").delete().eq("user_id", user_id).execute()
        print("  Deleted purchase orders.")
    except Exception as e:
        print("  Error deleting purchase orders:", e)

    try:
        db.table("manufacturing_runs").delete().eq("user_id", user_id).execute()
        print("  Deleted manufacturing runs.")
    except Exception as e:
        print("  Error deleting manufacturing runs:", e)

    try:
        db.table("sales_history").delete().eq("user_id", user_id).execute()
        print("  Deleted sales history.")
    except Exception as e:
        print("  Error deleting sales history:", e)

    try:
        db.table("inventory").delete().eq("user_id", user_id).execute()
        print("  Deleted inventory.")
    except Exception as e:
        print("  Error deleting inventory:", e)

    try:
        db.table("products").delete().eq("user_id", user_id).execute()
        print("  Deleted products.")
    except Exception as e:
        print("  Error deleting products:", e)

    try:
        db.table("suppliers").delete().eq("user_id", user_id).execute()
        print("  Deleted suppliers.")
    except Exception as e:
        print("  Error deleting suppliers:", e)

    try:
        db.table("warehouses").delete().eq("user_id", user_id).execute()
        print("  Deleted warehouses.")
    except Exception as e:
        print("  Error deleting warehouses:", e)

    # ── 1. Suppliers ─────────────────────────────────────────────────────
    supplier_rows = [
        {"name": n, "lead_time_days": lt, "reliability_score": rel, "user_id": user_id}
    ...
    try:
        db.table("warehouses").insert(warehouse_rows).execute()
        print("Inserted 3 warehouses.")
    except Exception as e:
        print("Error inserting warehouses:", e)


    # ── 2. Products ──────────────────────────────────────────────────────
    product_rows = [
        {"name": name, "category": cat, "unit_cost": cost, "unit_price": price, "user_id": user_id}
...
if __name__ == "__main__":
    # We fetch the first user to seed their data
    db = get_db()
